#######################################
##################################
### FASTQC: quality check of snATAC-seq data
...
#### Distinguish Index1, Read1, Read2 and Read3
        ...
        read3.fastq.paths <-fastq.paths[grep("_R3_001.fastq.gz", fastq.paths)]
################################################# generate zUMI script for each sample
        ...
        read3.fastqs <- paste(read3.fastq.paths, sep = "", collapse = " ")
...
#### Make directory for merged I1, R1, R2 and R3 files
...
################# Handle R3 stuff
# #### Make "R3 merged" directory
#         merged.R3.dir <- file.path(sample.output.dir, "merged_r3")
#         if(file.exists(merged.R3.dir)==F){
#             dir.create(merged.R3.dir)
#         }
#### Make the a merged R3 file of R3.fastq.gz files from this sample in this batch
        if(length(read3.fastq.paths) > 1){
            cat("Hold on... merging Read3 files for ", sample.names[j], "...\n", sep = "")
        } else {
            cat("Hold on... transferring large Read3 file for ", sample.names[j], "...\n", sep = "")
        }
        merged.R3.file <- file.path(merged.dir, paste(sample.names[j], "merged", sample.number, "L003_R3_001.fastq.gz", sep = "_"))
######## Check if sample has replicate data already run w/in loop: if TRUE: append the merged fastq; if FALSE: overwrite
        if((sample.names[j]%in%completed.sample.names)==TRUE){
            cat("Appending the reads from sequencing replicates.\n")
            system(paste("cat ",read3.fastqs, " >> ", merged.R3.file, sep = ""))
        } else {
            system(paste("cat ",read3.fastqs, " > ", merged.R3.file, sep = ""))
        }
################################### Print the script
...
## Input
        ...
        cmd.out <- paste0(cmd.out, "fastqc ", read3.fastqs, " -o ", sample.output.dir, "\n")
